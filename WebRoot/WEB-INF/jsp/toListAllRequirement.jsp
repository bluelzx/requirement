...
<head>
    ...
    <script type="text/javascript">
        ...
        function toCloseRequirement(){
        	...
        	if(!toClose){
        		$.ligerDialog.error('亲,请选择要关闭的需求!');
        		return;
        	}else{
        		
        		if(toClose.owner!=loginUser){
        			$.ligerDialog.error('亲,你没有权限关闭需求：'+toClose.requirementName);
        			return;
        		}
        		
        		if(toClose.status==4){
        			$.ligerDialog.error('亲,需求：'+toClose.requirementName+',已经关闭,不用再次关闭!');
        			return;
        		}
        		if(toClose.status!=3){
        			$.ligerDialog.error('亲,需求：'+toClose.requirementName+',有未完成的任务,不能关闭!');
        			return;
        		}
        	}
        	...
        	$.ligerDialog.confirm('亲,确认要关闭需求：'+toCloseName, function (yes) { 
			                		 
			                		if(yes){
			                			$.ajax({
								                type: 'POST',
								                url: '<%=path%>/closeByRequirementId.do',
								                data: 'requirementId='+toCloseId+'&firstCloseTime='+toClose.closeTime,
								                dataType: 'json',
								                beforeSend: function ()
								                {
								                    $("#pageloading").show();
        ...
        $(function () {
           ...
           $.ajax({	type: 'POST',
            ...
            mygrid = $("#maingrid4").ligerGrid({
                ...
                url: '<%=path%>/listAllRequirement.do?status='+$("#status").val(),
                root: 'rows',
                record: 'total',
        ...
        function f_showTasks(row, detailPanel,callback)
        {
            ...
            $(grid).css({'margin':10,'border-color':'orange','border-width':5}).ligerGrid({
                ...
                { display: '创建者', name: 'creator', width: 100,align:'center' }
        ...
        function f_search()
        {	
        	mygrid.options.url = "<%=path%>/listAllRequirement.do";
 ...
 <table cellpadding="0" cellspacing="0" class="l-table-edit" >
            <tr>
               ...
               <td align="right" class="l-table-edit-td">需求部门:</td>
                <td align="left" class="l-table-edit-td" style="width:160px"><input name="dept" type="text" id="dept" ltype="text"  /></td>
            ...
            <td align="right" class="l-table-edit-td">需求提出者:</td>
                <td align="left" class="l-table-edit-td" style="width:160px"><input name="presenter" type="text" id="presenter" ltype="text"  /></td>
       
       <td align="right" class="l-table-edit-td">责任人:</td>
                <td align="left" class="l-table-edit-td" style="width:160px"><input name="owner" type="text" id="owner" ltype="text"  /></td>
